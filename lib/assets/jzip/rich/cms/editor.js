...
Rich.Cms.Editor = (function() {
  var content_class        = "rich_cms_content", mark_class = "marked", edit_panel = "#rich_cms_panel",
      ...
      cleditor_images_path = "/images/rich_cms/cleditor",
  ...
  var edit = function() {
      ...
      if (attr.match(/^data-/)) {
        ...
        if (attr == specs.value) {
          ...
          switch (editable_input_type) {
            ...
            case "html":
              inputs.append("<textarea id='cleditor_input' name='" + name + "' style='width: 500px; height: 250px'>" + value + "</textarea>"); break;
          }
        } else {
    ...
    setTimeout(function() {
      if ($("#cleditor_input").length) {
        $("#cleditor_input").cleditor()[0].focus();
      }
    }, 250);
  };
