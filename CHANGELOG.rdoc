...
* Allow no wrapping tag in cases when no user is logged in (handy in case the extra 'span' will bork the layout). Thanks Jeroen Bulters (@bulters) for contributing

== Version 3.0.1 (April 5, 2011)
...
* Make sure html options get passed to tags when not in edit mode. Thanks Jeroen Bulters (@bulters) for contributing
* Allow no wrapping tag in cases when no user is logged in. Thanks Jeroen Bulters (@bulters) for contributing
